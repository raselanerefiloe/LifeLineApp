    ...
    <div class="container mx-auto py-12 px-6 lg:px-8">
        ...
        <div class="relative bg-green-600 text-white text-center py-16 md:py-24 overflow-hidden rounded-lg">
            ...
            <div class="absolute md:hidden lg:hidden inset-0 z-0">
                <img src="{{ asset('img/hero/lg_product.jpg') }}" alt="Background" class="w-full h-full object-cover opacity-100" />
            </div>
        </div>
                ...
                <form method="GET" action="{{ route('product.index') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="brand" name="brand" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            <option value="brand-a" {{ request('brand') == 'brand-a' ? 'selected' : '' }}>Brand A</option>
                            <option value="brand-b" {{ request('brand') == 'brand-b' ? 'selected' : '' }}>Brand B</option>
                            <option value="brand-c" {{ request('brand') == 'brand-c' ? 'selected' : '' }}>Brand C</option>
                            <option value="brand-d" {{ request('brand') == 'brand-d' ? 'selected' : '' }}>Brand D</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="range" id="price" name="price" min="0" max="500" step="10" value="{{ request('price') }}" class="mt-1 w-full">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="availability" name="availability" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            <option value="pre-order" {{ request('availability') == 'pre-order' ? 'selected' : '' }}>Pre-order</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="color" name="color" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="size" name="size" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            <option value="x-large" {{ request('size') == 'x-large' ? 'selected' : '' }}>X-Large</option>
                        </select>
                    </div>

                    <!-- Apply Filters Button -->
                    <div class="mb-4">
                        <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg shadow-md hover:bg-green-700">Apply Filters</button>
                    </div>
                </form>
            </aside>
            ...
            <div class="md:col-span-3">
                ...
                <div class="mb-6">
                    <form class="relative" method="GET" action="{{ route('product.index') }}">
                        <input type="text" placeholder="Search products..." name="search" class="w-full border border-gray-300 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-green-500" value="{{ request('search') }}">
                        <button type="submit" class="absolute inset-y-0 right-0 px-4 py-2 bg-green-600 text-white rounded-r-lg hover:bg-green-700">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 4a7 7 0 00-7 7 7 7 0 0014 0 7 7 0 00-7-7zm0 14a9 9 0 01-9-9 9 9 0 1118 0 9 9 0 01-9 9z" />
                            </svg>
                        </button>
                    </form>
                </div>
                    ...
                    @foreach($products as $product)
                        <div class="relative bg-white rounded-lg shadow-md overflow-hidden group">
                            ...
                            <div class="p-6">
                                ...
                                <p class="mt-4 text-green-600 font-semibold">R{{ number_format($product->price, 2) }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
